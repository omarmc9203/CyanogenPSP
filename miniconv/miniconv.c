 ...
 ****************************************/
static int
utf16le_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  int count = 0;
  if (n >= 2) {
    ucs4_t wc = s[0] + (s[1] << 8);
 ...
 ****************************************/
static int
utf16be_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  int count = 0;
  if (n >= 2) {
    ucs4_t wc = (s[0] << 8) + s[1];
...
static int
ces_big5_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  /* Code set 0 (ASCII) */
  if (c < 0x80)
...
static int
jisx0208_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c1 = s[0];
...
static int
sjis_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  if (c < 0x80 || (c >= 0xa1 && c <= 0xdf))
    return jisx0201_mbtowc(pwc,s,n);
  else {
    unsigned char s1, s2;
    s1 = c;
...
static int
euc_kr_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
...
static int
cp1250_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  if (c < 0x80) {
    *pwc = (ucs4_t) c;
 ...
 *      convert to UTF-8                *
 ****************************************/
#define CONVERT_BUFFER_SIZE 2048
char convert_buffer[CONVERT_BUFFER_SIZE+1];
...
char* utf16le_to_utf8(const unsigned short* utf16le) {
	if (utf16le == NULL)
		return NULL;
	int ilen = 0;
	while(utf16le[ilen] != 0)
		ilen++;
	if (ilen == 0)
		return NULL;
	ilen *= 2;
		
	unsigned char* p = (unsigned char*)utf16le;
	...
	j = 0; 
	while( i < ilen) {
		ret_val = utf16le_mbtowc(&ucs4_char, p+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* utf16be_to_utf8(const unsigned short* utf16be) {
	if (utf16be == NULL)
		return NULL;
	int ilen = 0;
	while(utf16be[ilen] != 0)
		ilen++;
	if (ilen == 0)
		return NULL;
	ilen *= 2;
		
	unsigned char* p = (unsigned char*)utf16be;
	...
	j = 0; 
	while( i < ilen) {
		ret_val = utf16be_mbtowc(&ucs4_char, p+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* gbk_to_utf8(const unsigned char* gbk){
	...
	j = 0; 
	while( i < ilen) {
		ret_val = ces_gbk_mbtowc(&ucs4_char, gbk+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* big5_to_utf8(const unsigned char* big5) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = ces_big5_mbtowc(&ucs4_char, big5+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* sjis_to_utf8(const unsigned char* sjis) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = sjis_mbtowc(&ucs4_char, sjis+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* euc_kr_to_utf8(const unsigned char* euc_kr) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = euc_kr_mbtowc(&ucs4_char, euc_kr+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* ms_ee_to_utf8(const unsigned char* ms_ee) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = cp1250_mbtowc(&ucs4_char, ms_ee+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* ms_cyrl_to_utf8(const unsigned char* ms_cyrl) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = cp1251_mbtowc(&ucs4_char, ms_cyrl+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
...
char* iso8859_2_to_utf8(const unsigned char* iso8859_2) {
	...
	j = 0; 
	while( i < ilen) {
		ret_val = iso8859_2_mbtowc(&ucs4_char, iso8859_2+i, ilen-i);
		if ( ret_val < 0 ) { 
			i++;
			convert_buffer[j] = '?';
			j++;
